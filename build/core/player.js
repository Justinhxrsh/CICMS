...
class Player {
  constructor(id, name, ws) {
    ...
    // Position (tile-based)
    this.col = _constants.SPAWN.col + Math.floor(Math.random() * 3) - 1;
    this.row = _constants.SPAWN.row + Math.floor(Math.random() * 3) - 1;

    // Pixel position (for smooth interpolation)
    const px = (0, _utils.tileToPixel)(this.col, this.row);
    this.x = px.x;
    this.y = px.y;
    ...
    // Skills (XP values)
    this.skills = {};
    _constants.SKILLS.forEach(skill => {
      this.skills[skill] = 0;
    });
  ...
  addItem(itemDefKey, quantity = 1) {
    ...
    if (!def) return false;
    if (this.inventory.length >= _constants.GAME.MAX_INVENTORY && !def.stackable) return false;
    if (def.stackable) {
    ...
    if (this.inventory.length >= _constants.GAME.MAX_INVENTORY) return false;
    this.inventory.push({
  ...
  unequip(slot) {
    ...
    if (this.inventory.length >= _constants.GAME.MAX_INVENTORY) {
      return {
        success: false,
  ...
  // Gain skill XP
  gainXp(skill, amount) {
    if (!_constants.SKILLS.includes(skill)) return;
    const oldLevel = (0, _utils.xpToLevel)(this.skills[skill]);
    this.skills[skill] += amount;
    const newLevel = (0, _utils.xpToLevel)(this.skills[skill]);
  ...
  consume(itemId) {
    ...
    if (!item || item.type !== 'consumable') return {
      success: false,
      message: 'Cannot consume that item.'
    };
    const def = _constants.ITEM_DEFS[item.defKey];
    if (def && def.heals) {
    ...
    return {
      success: true,
      message: `Used ${item.name}. HP restored.`
    };
  }
  ...
  update(deltaMs) {
    ...
    const speed = _constants.GAME.PLAYER_SPEED * _constants.GAME.TILE_SIZE * (deltaMs / 1000);
    if (dist <= speed) {
      // Reached tile
      this.x = targetPx.x;
      this.y = targetPx.y;
