...
export class Delaford {
    ...
    handleJoin(ws, data) {
        ...
        if (!playerName || playerName.length < 2 || playerName.length > 20) {
            sendMessage(ws, { type: 'ERROR', message: 'Invalid player name (2-20 chars).' });
            return;
        }
    ...
    handleAction(ws, data) {
        ...
        const { action, targetId, itemId, quantity } = data;
        ...
        switch (action) {
            ...
            default:
                sendMessage(ws, { type: 'ERROR', message: `Unknown action: ${action}` });
        }
    }
    ...
    handleCommand(ws, data) {
        ...
        if (command === 'give') {
            ...
            if (added > 0) {
                this.sendActionResult(ws, true, `Gave ${added}x ${ITEM_DEFS[itemId].name}.`);
            } else {
            ...
            if (isNaN(col) || isNaN(row) || col < 0 || col >= GAME.MAP_COLS || row < 0 || row >= GAME.MAP_ROWS) {
    ...
    handleInteract(ws, data) {
        ...
        if (targetType === 'npc') {
            ...
            if (player.addItem(item.defKey)) {
                ...
                this.broadcast({ type: 'ITEM_REMOVED', itemId: item.id });
                this.sendInventoryUpdate(ws, player);
    ...
    handleGather(ws, data) {
        ...
        const { worldItemId, zoneKey } = data;
        ...
        if (zoneDef.requiredTool) {
            const hasTool = player.inventory.some(i => i.defKey === zoneDef.requiredTool.toUpperCase()) ||
                Object.values(player.equipment).some(i => i && i.defKey === zoneDef.requiredTool.toUpperCase());
            if (!hasTool) {
        ...
        this.broadcast({ type: 'ITEM_REMOVED', itemId: worldItem.id });
    ...
    handleChat(ws, data) {
        ...
        this.broadcast({ type: 'CHAT_MESSAGE', ...chatMsg });
    }
    ...
    sendActionResult(ws, success, message) {
        sendMessage(ws, { type: 'ACTION_RESULT', success, message });
    }

    addActionLog(ws, message) {
        sendMessage(ws, { type: 'ACTION_LOG', message });
    }
    ...
    startGameLoop() {
        ...
        const SYNC_INTERVAL = GAME.SYNC_INTERVAL;

        this.gameLoopInterval = setInterval(() => {
            const tickResult = this.world.tick();
            const { updatedNPCs, updatedPlayers, survival } = tickResult;
            const now = Date.now();
