import { generateId, tileToPixel, findPath, randomChoice, isTileWalkable } from '../shared/utils.js';
import { GAME, NPC_DEFS, ITEM_DEFS, SHOP_BUY_MARKUP, SHOP_SELL_DISCOUNT } from '../shared/constants.js';

export class NPC {
    constructor(defKey, col, row) {
        this.id = generateId();
        this.defKey = defKey;
        const def = NPC_DEFS[defKey];
        ...
        if (def.shop) {
            def.shop.forEach(itemKey => {
                const itemDef = ITEM_DEFS[itemKey];
                this.shopItems.push({
                    defKey: itemKey,
                    name: itemDef.name,
                    emoji: itemDef.emoji,
                    color: itemDef.color,
                    type: itemDef.type,
                    buyPrice: Math.ceil(itemDef.value * SHOP_BUY_MARKUP),
                    sellPrice: Math.floor(itemDef.value * SHOP_SELL_DISCOUNT),
    ...
    updateWander(deltaMs) {
        ...
        if (this.wanderTimer >= this.WANDER_INTERVAL) {
            ...
            for (let i = 0; i < attempts; i++) {
                ...
                if (isTileWalkable(newCol, newRow)) {
                    const path = findPath(this.col, this.row, newCol, newRow);
                    if (path.length > 1) {
                        this.path = path;
        ...
        const speed = GAME.NPC_SPEED * GAME.TILE_SIZE * (deltaMs / 1000);
    ...
    processBuy(player, itemDefKey, quantity = 1) {
        if (this.type !== 'shop') {
            return { success: false, message: 'This NPC does not run a shop.' };
        }
        ...
        if (player.inventory.length >= GAME.MAX_INVENTORY) {
    ...
    processSell(player, itemId, quantity = 1) {
        if (this.type !== 'shop') {
            return { success: false, message: 'This NPC does not buy items.' };
        }
        ...
        const sellPrice = Math.floor(item.value * SHOP_SELL_DISCOUNT) * quantity;
        const removed = player.removeItem(itemId, quantity);
        if (!removed) {
            return { success: false, message: 'Could not remove item.' };
        }

        player.gold = Math.min(player.gold + sellPrice, GAME.MAX_GOLD);
    ...
    processBank(player, action, itemId, quantity) {
        if (this.type !== 'bank') {
            return { success: false, message: 'This is not a bank.' };
        }
        ...
        if (action === 'DEPOSIT') {
            ...
            const itemDef = ITEM_DEFS[item.defKey];
            const sellValue = (itemDef ? itemDef.value : item.value) * qty;

            const removed = player.removeItem(itemId, qty);
            if (!removed) return { success: false, message: 'Failed to deposit/sell.' };

            player.gold = Math.min(player.gold + sellValue, GAME.MAX_GOLD);
            // We no longer put it into player.bank, effectively acting as an instant direct 'Sell for Gold' action

            return { success: true, message: `Deposited ${item.name} into Bank for ${sellValue} gold!` };
        }

        if (action === 'WITHDRAW') {
            const bankItem = player.bank.find(i => i.id === itemId);
            if (!bankItem) return { success: false, message: 'Item not in bank.' };
            if (player.inventory.length >= GAME.MAX_INVENTORY && !bankItem.stackable) {
                return { success: false, message: 'Inventory full.' };
            }
            ...
            return { success: true, message: `Withdrew ${bankItem.name}.` };
        }

        return { success: false, message: 'Unknown bank action.' };
    }
